...
char top; 
	while (!stackEmpty(s)) 			//prechádza zásobníkom
	{
		stackTop(s, &top); // znak z vrchu zásobníka uloží do premennej
		stackPop(s); 				// popnutie znaku z vrchu zasobnika
		if (top == '(')	
		{
			break; 					//ukoncenie ak nacitalo (
		}
		postExpr[(*postLen)++] = top; //inkrementuje dlzku vystupneho retazca a vlozenie znaku z vrchu zasobnika na jeho koniec
	}
}
...
if (stackEmpty(s)) 
	{
		stackPush(s, c);		// vloží operátor na vrch zásobníka
		return;
	}

	stackTop(s, &top); // uložení znaku z vrcholu zásobníku
	if (top=='(' || ((top == '+' || top == '-') && (c == '*' || c == '/')))
	{ 
		stackPush(s, c); 
		return;
	...
	postExpr[(*postLen)++] = top; //inkrementuje dlzku vystupneho retazca a vlozenie znaku z vrchu zasobnika na jeho koniec
	stackPop(s); 				// popnutie vrchu zásobníka
	doOperation(s, c, postExpr, postLen); //rekurzívne volanie funkcie 
}
...
char* infix2postfix (const char* infExpr) {
	...
	tStack *stack = (tStack *) malloc(sizeof(tStack)); //alokuje pamat pre zasobnik
	if (!stack)
	{
		free(postExpr);
		return NULL;
	...
	for (char c = *infExpr; c != '\0'; c = *(++infExpr))	 //prechádza celý vstupny retazec
	{
		if ((c >= '0' && c <= '9') || (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')) 	//operand
		{
			postExpr[postLen++] = c; 		//inkrementuje dlzku vystupneho retazca a vlozenie znaku z vrchu zasobnika na jeho koniec
		}
		else if (c == '(') 
		{
			stackPush(stack, c); 	//pushne c na vrchol zasobnika
		}
		else if (c == ')') 
		{
			untilLeftPar(stack, postExpr, &postLen);	 //vyprazdni stack az po lavu zatvorku
		}
		...
		{
			while (!stackEmpty(stack)) 		//prechadza zasobnikom
			{
	...
	postExpr[postLen++] = '\0'; 	//ukončenie vystupneho retazca
	free(stack); 	//uvolni alokovanu pamat pre zasobnik
	return postExpr; 	//funkcia vráti výsledný retazec
}
